	...
	<div class="container" id="containe">
		<div>
			<form class="form-inline" name="search" class="search"  method="post"
				action="<%=request.getContextPath()%>/sellre.action?act=query">
		...
		<!--增加-->
			<div id="div_entering">
				<form action="<%=request.getContextPath()%>/sellre.action?act=selladd"
				method="post">
					<div class="form-group">
						<label>客户编号：</label>
						...
						<c:forEach items="${list }" var="sellre">
							<tr>
								...
								<td><a class=" set btn btn-warning btn-sm" type="button"role=button  
									href="${pageContext.request.contextPath }/sellre.action?act=update&sellreid=${sellre.sellreid}">
									修改</a>
									<a class="btn btn-danger btn-sm" type="button" role=button 
									href="${pageContext.request.contextPath }/sellre.action?act=delete&sellreid=${sellre.sellreid}"
